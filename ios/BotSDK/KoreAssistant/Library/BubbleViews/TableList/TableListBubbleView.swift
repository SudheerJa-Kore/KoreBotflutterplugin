...
extension TableListBubbleView: UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if elements?.bgcolor != nil{
            cell.bgView.backgroundColor = UIColor.init(hexString: (elements?.bgcolor) ?? "#FFFFFF")
        }
        ...
        let hexString: String = (elements?.title?.rowColor!.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines))!
           if (hexString.hasPrefix("#")) {
                 cell.titleLabel.textColor = UIColor.init(hexString: (elements?.title?.rowColor) ?? "#26344A")
                 cell.subTitleLabel.textColor = UIColor.init(hexString: (elements?.title?.rowColor) ?? "#26344A")
                 cell.priceLbl.textColor =   UIColor.init(hexString: (elements?.title?.rowColor) ?? "#26344A")
            }else{
