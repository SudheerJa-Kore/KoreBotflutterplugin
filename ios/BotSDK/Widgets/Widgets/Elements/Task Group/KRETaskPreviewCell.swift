...
open class KRETaskPreviewCell: UITableViewCell {
    // MARK: - properties
    let bundle = KREResourceLoader.shared.resourceBundle()
    public var itemSelectionHandler:((KRETaskPreviewCell) -> Void)?
    public var moreSelectionHandler:((KRETaskPreviewCell) -> Void)?
   ...
   public lazy var selectionView: UIButton = {
    ...
    func initialize() {
      ...
      //  containerView.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:[dateTimeLabel]-16-[triangle(8)]", options: [], metrics: nil, views: views))
        ...
        containerViewLeadingConstraint = NSLayoutConstraint(item:containerView, attribute:.leading, relatedBy:.equal, toItem:contentView, attribute:.leading, multiplier:1.0, constant: 0.0)
        contentView.addConstraint(containerViewLeadingConstraint)
        containerViewTrailingConstraint = NSLayoutConstraint(item:contentView, attribute:.trailing, relatedBy:.equal, toItem:containerView, attribute:.trailing, multiplier:1.0, constant: 0.0)
        contentView.addConstraint(containerViewTrailingConstraint)
...
// MARK: - KRETriangleView
class KRETriangleView: UIView {
    // MARK: - init
    ...
    override func draw(_ rect: CGRect) {
        ...
        context.setFillColor(UIColor.blueyGrey.cgColor)
        context.fillPath()
    }
}
