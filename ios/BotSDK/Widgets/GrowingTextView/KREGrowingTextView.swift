...
open class KREGrowingTextView: UIScrollView {
    ...
    private func resetPlaceholderLabelFrame() {
        ...
        //let placeholderSize = CGSize(width: UIScreen.main.bounds.size.width - 120, height: 21.0)
        _placeholderLabel.frame = CGRect(origin: CGPoint(x: _textView.textContainerInset.left + 5, y: _textView.textContainerInset.top + 2), size: placeholderSize)
    }
    
    private func measureTextViewSize() -> CGSize {
        let rect = self.bounds
        var size = _textView.sizeThatFits(CGSize(width: rect.size.width, height: CGFloat.greatestFiniteMagnitude))
    ...
    private func measureFrame(_ contentSize: CGSize) -> CGRect {
        ...
        var _frame = frame
        _frame.size.height = selfSize.height //- 3.0
        return _frame
    }
