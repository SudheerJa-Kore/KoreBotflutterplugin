    ...
    public override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (option.optionType == KREOptionType.button) {
            ...
            if #available(iOS 8.2, *) {
                cell.textLabel?.font = UIFont.textFont(ofSize: 15.0, weight: .bold)
            } else {
...
//        self.imageView.addConstraint(self.imageViewHeightConstraint)
        ...
        if #available(iOS 8.2, *) {
            subTitleLabel.font = UIFont.textFont(ofSize: 14.0, weight: .regular)
        } else {
            // Fallback on earlier versions
        }
        ...
        if #available(iOS 8.2, *) {
            hashTagsLabel.font = UIFont.textFont(ofSize: 14.0, weight: .regular)
        } else {
            // Fallback on earlier versions
        }
        ...
        if #available(iOS 8.2, *) {
            informationLabel.font = UIFont.textFont(ofSize: 12.0, weight: .regular)
        } else {
            // Fallback on earlier versions
        }
        ...
        if #available(iOS 8.2, *) {
            statusLabel.font = UIFont.textFont(ofSize: 12.0, weight: .regular)
        } else {
            // Fallback on earlier versions
        }
        ...
        let views = [ "containterView": containterView, "optionsView": self.optionsView, "titleLabel": titleLabel, "subTitleLabel": subTitleLabel, "hashTagsLabel":hashTagsLabel, "informationLabel": informationLabel, "statusLabel": statusLabel] as [String : Any]
        ...
        self.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|[containterView]-(>=10@1)-[optionsView]|", options: [], metrics: nil, views: views))
...
//        self.imageView.setImageWith(NSURL(string: cardInfo.imageURL!) as URL!, placeholderImage: UIImage.init(named: "placeholder_image"))
        ...
        if let sharesCount = cardInfo.payload?["nShares"] as? Int {
        ...
        let count: Int = min(cardInfo.options?.count ?? 0, KACardView.buttonLimit)
...
public class KACarouselView: KRECarouselView {
    ...
    public override func getExpectedHeight(cardInfo: KRECardInfo, width: CGFloat) -> CGFloat {
        ...
        let count: Int = min(cardInfo.options?.count ?? 0, KRECardView.buttonLimit)
