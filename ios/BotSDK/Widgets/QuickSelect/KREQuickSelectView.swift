...
public class Word: NSObject {
    static let titleCharLimit: Int = 100
    ...
    public init(title: String, payload: String, imageURL: String, showMoreMessages: String, type: String, url: String) {
        super.init()
        self.title = truncateString(title, count: Word.titleCharLimit)
        self.payload = payload
        self.imageURL = imageURL
        
        self.showMoreMessages = showMoreMessages
        self.type = type
        self.url = url
    }
...
open class KREQuickSelectView: UIView {
  ...
  open lazy var collectionView: UICollectionView = {
    ...
    public var sendQuickReplyLinkAction: ((_ text: String?) -> Void)?
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor.clear //paleGrey
...
extension KREQuickSelectView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let word = words?[indexPath.row] {
            if word.type == "web_url"{
                self.sendQuickReplyLinkAction?(word.url)
            }else{
                var payload = word.payload
                if payload == nil{
                    payload = word.title
                }
                sendQuickReplyAction?(word.title, payload)
            }
